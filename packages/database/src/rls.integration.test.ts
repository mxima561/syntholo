...
describe.sequential("capability role provisioning migration", () => {
  ...
  it("applies fresh, reuses safe cluster roles in a second owned database, and reruns as a non-superuser CREATEROLE actor", async () => {
    ...
    try {
      ...
      expect(journals.map(({ rows }) => rows[0]?.count)).toEqual(["7", "7"]);
...
describe("PostgreSQL account role boundary", () => {
  ...
  it("upgrades the exact populated 0001-0003 journal and applies all migrations fresh", async () => {
    ...
    try {
      ...
      expect(afterUpgrade.rows[0]?.count).toBe("7");
      await migrateDatabase(upgradeDb);
      const afterRerun = await upgradeDb.pool.query<{ count: string }>(
        "select count(*)::text as count from drizzle.__drizzle_migrations",
      );
      expect(afterRerun.rows[0]?.count).toBe("7");
      ...
      expect(freshJournal.rows[0]?.count).toBe("7");
    } finally {
      await Promise.allSettled([
        upgradeDb?.close(),
        freshDb?.close(),
