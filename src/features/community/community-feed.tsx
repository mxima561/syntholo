...
const demoReferenceTime = new Date("2026-08-11T20:00:00.000Z").getTime();

function relativeDay(createdAt: string) {
  const days = -Math.max(1, Math.round((demoReferenceTime - new Date(createdAt).getTime()) / 86_400_000));
...
export function CommunityFeed({ initialPosts }: { initialPosts: CommunityPost[] }) {
  ...
  function publish() {
    ...
    setPosts((items) => [{ id: `post-${items.length + 1}`, space: space === "All spaces" ? "Implementation Wins" : space, authorName: "Maria Chen", authorRole: "Founder", businessName: "Northstar Advisory", initials: "MC", title: title.trim(), body: body.trim(), createdAt: new Date().toISOString(), commentCount: 0, reactionCount: 0, status: "published" }, ...items]);
