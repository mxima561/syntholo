...
Clerk handles member magic links, Google/Microsoft or other approved social sign-in, account claim, and consumer sessions. Cloudflare Access asserts staff identity at the admin origin; the `staff` table authorizes role and status. The providers are deliberately not unified. The admin origin is not publicly routable; identity is asserted by Cloudflare Access and authorized against the staff table.

Member middleware cannot authorize coach/admin routes, and staff middleware cannot silently act as a member. A Clerk student session presented to the admin app is meaningless and is rejected. If a person has both relationships, the identities and sessions remain separate. Every authenticated request resolves an immutable internal actor, account or staff scope, role, and effective authorization before reaching a repository.
...
The production API is the sole business-write authority. The web interface contains no database credentials, Stripe secret, Access JWT secrets (none — verification uses the public JWKS), Circle admin token, HighLevel credential, Mux secret, Resend key, Blob write token, or staff-only business logic.
...
| Member identity | Clerk | Consumer sign-in, claim, social/magic-link identity, sessions, and Circle OAuth provider. Student app only (`apps/web`). |
| Staff identity | Cloudflare Access | Identity-aware proxy in front of `apps/admin`. JWT verified against team JWKS; `staff` table authorizes role/status. Mandatory MFA in the Access policy. Origin locked with Cloudflare Tunnel. |
...
- Verify Clerk and Stripe signatures, and Cloudflare Access JWTs (`aud`, `iss`, `exp`) against the team JWKS. Never trust an unverified Access header.
- Stolen admin device: revoke the user in Cloudflare Access. That kills the session at the proxy, not just the app.
...
- **Clerk unavailable:** retain public cached content; do not bypass authorization. Show provider-specific recovery guidance.
- **Cloudflare Access unavailable:** the admin origin stays closed. Do not enable the local bypass in production. Follow the break-glass procedure in the runbook.
